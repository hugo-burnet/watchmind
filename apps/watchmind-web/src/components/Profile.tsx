...
export function Profile() {
  ...
  return <>
    ...
    <section className="profile-map" aria-labelledby="map-title"><header><div><p className="eyebrow">Familles de goûts</p><h2 id="map-title">Vos pôles</h2></div><p>{selected.profile.mode === "clustered" ? "Chaque bloc regroupe des favoris qui partagent les mêmes thèmes. Sa taille indique le nombre d’œuvres." : "Le profil reste regroupé tant que l’historique est court."}</p></header>
      <div className="profile-map__canvas">{selected.profile.poles.map((pole) => <article className="profile-pole" key={pole.ordinal}><div className="profile-pole__count"><strong>{pole.member_count}</strong><span>œuvre{pole.member_count > 1 ? "s" : ""}</span></div><div><p className="eyebrow">Pôle {pole.ordinal + 1}</p><h3>{pole.dominant_tags.slice(0, 3).map((tag) => tag.name).join(" · ")}</h3><p>{pole.representative_work_ids.map((id) => workNames.get(id) ?? `œuvre ${id}`).join(", ")}</p></div></article>)}</div>
